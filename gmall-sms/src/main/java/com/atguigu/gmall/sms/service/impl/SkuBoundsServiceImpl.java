    ...
    public List<ItemSaleVO> queryItemSaleVOs(Long skuId) {
       ...
       List<ItemSaleVO> itemSaleVOS = new ArrayList<>();
        ...
        if(!CollectionUtils.isEmpty(skuBoundsEntities)){
            ...
            saleVO.setType("积分");
            BigDecimal buyBounds = skuBoundsEntities.get(0).getBuyBounds();//购买积分
            BigDecimal growBounds = skuBoundsEntities.get(0).getGrowBounds();//成长积分
            saleVO.setDecs("购物积分赠送" + buyBounds.intValue() + ", 成长积分赠送" + growBounds.intValue());
        ...
        if(!CollectionUtils.isEmpty(skuFullReductionEntities)){
            ...
            saleVO.setDecs("满" + fullPrice.intValue() + "减" + reducePrice.intValue());
            itemSaleVOS.add(saleVO);
        }
        ...
        if(!CollectionUtils.isEmpty(skuLadderEntities)){
            ...
            saleVO.setDecs("满" + fullCount + "件,打" + reducePrice.divide(new BigDecimal(10)).floatValue() + "折");
            itemSaleVOS.add(saleVO);
        }
